...
If a task editor is currently open in active scope, the bottom bar SHALL instead show the edit-mode action set (`Move`, `Delete`, `More`) and MUST NOT show `+ Section`.

If a task editor is currently open in `trash` scope, the bottom bar SHALL instead show `More` and MUST NOT show `+ Section`.
...
#### Scenario: Active-scope edit-mode action group replaces bottom bar actions when an editor is open
- **WHEN** the user is viewing `/projects/:projectId`
- **AND** a task editor is currently open in active scope
...
#### Scenario: Trash-scope edit-mode only shows More on the Project page
- **WHEN** the user is viewing `/projects/:projectId?scope=trash`
- **AND** a task editor is currently open in `trash` scope
- **THEN** the content bottom bar displays `More`
- **AND** the content bottom bar MUST NOT display `+ Section`
- **AND** the content bottom bar MUST NOT display `Move`
- **AND** the content bottom bar MUST NOT display `Delete`
- **AND** the content bottom bar MUST NOT display `Schedule`
- **AND** the content bottom bar MUST NOT display `Search`
