...
- **THEN** Areas SHALL be ordered by title (case-insensitive)
- **AND THEN** Projects within each group (unassigned or an Area) SHALL be ordered by title (case-insensitive)

#### Scenario: Manual ordering activates per scope
- **WHEN** the user manually reorders Areas for the first time
- **THEN** subsequent Sidebar loads SHALL use the persisted Area order instead of alphabetical order
- **AND WHEN** the user manually reorders Projects within a specific group for the first time
- **THEN** subsequent Sidebar loads SHALL use the persisted order for that group instead of alphabetical order
