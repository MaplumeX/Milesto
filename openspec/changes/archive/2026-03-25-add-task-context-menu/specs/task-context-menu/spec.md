...
When the task context menu root is open, it MUST show exactly four actions:
- `Plan` / `Schedule`
- `Tags`
- `Due`
- `Complete` or `Restore`

The final completion action label MUST reflect the current task status:
- open tasks show `Complete`
- done tasks show `Restore`

#### Scenario: Open task shows Complete in the root menu
- **WHEN** the task context menu root is open for an open task
- **THEN** the root menu shows `Schedule`, `Tags`, `Due`, and `Complete`
...
Choosing `Complete` or `Restore` from the task context menu root MUST persist the task status change immediately.
...
- **WHEN** the user opens the task context menu root for a done task
- **AND** the user chooses `Restore`
- **THEN** the task is persisted with `status=done -> open`
- **AND THEN** the task context menu closes
