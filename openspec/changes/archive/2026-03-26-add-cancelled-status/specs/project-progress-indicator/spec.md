...
The system SHALL provide an API that returns per-project task progress counts for a set of Projects so that list views can render progress without N IPC calls.
...
- `done_count`: number of matching tasks with `status IN (done, cancelled)`
- `total_count`: number of matching tasks with `status IN (open, done, cancelled)`
...
#### Scenario: Batch counts reflect all active and terminal task states
- **WHEN** the UI requests progress counts for project id `P1`
- **AND WHEN** P1 has 3 non-deleted open tasks, 1 non-deleted done task, and 1 non-deleted cancelled task
...
The control MUST NOT render any terminal-state icon unless the project status is `done` or `cancelled`.
...
#### Scenario: Open project renders pie fill for partial closed progress
- **WHEN** a project has `status = open`
- **AND WHEN** `total_count > 0`
...
#### Scenario: Open project at 100% closed progress is distinguishable from closed
- **WHEN** a project has `status = open`
- **AND WHEN** `total_count > 0`
...
- **AND THEN** the control is visually distinguishable from `status = cancelled`
- **AND THEN** the control does not display a terminal-state icon
...
#### Scenario: Cancelled project renders as x state with hidden border and preserved gap
- **WHEN** a project has `status = cancelled`
- **THEN** the progress control renders in the cancelled style
- **AND THEN** the control displays an `x`
- **AND THEN** the control's outer border is visually hidden (transparent)
- **AND THEN** the gap ring between border and fill remains visible as background
...
If the project status is `done` or `cancelled`, activating the control SHALL reopen the project without confirmation.

Activating the progress control SHALL NOT cancel a project.

In the Sidebar open projects list, the system SHALL render a display-only progress indicator.

- The Sidebar indicator MUST NOT complete, cancel, or reopen the project.
