...
- **WHEN** the user enters valid repository configuration and valid credentials in Settings
...
- **WHEN** the user provides an invalid endpoint, bucket, prefix, or credential set
- **AND** the user runs connection validation
...
- **AND** the user attempts to enable synchronization
...
The system MUST provide a Settings entry for synchronization that exposes:
...
- **THEN** Settings MUST show that sync is not currently running
- **AND** Settings MUST show the updated last successful sync time

#### Scenario: Failed sync shows actionable status
- **WHEN** a synchronization cycle fails
- **THEN** Settings MUST show that sync is in an error state
- **AND** Settings MUST show the most recent error using `code` and `message`
