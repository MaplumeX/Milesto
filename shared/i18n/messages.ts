import type { Locale } from './locale'

// Shared message catalogs used by both Renderer (i18next resources) and Main (native dialogs).
// Keep these small and predictable: no runtime disk reads, no IPC transfer of catalogs.

export type MessageCatalog = Record<string, unknown>

export const messagesEn = {
  common: {
    none: 'None',
    noneOption: '(none)',
    all: 'All',
    clear: 'Clear',
    close: 'Close',
    retry: 'Retry',
    rename: 'Rename',
    delete: 'Delete',
    confirm: 'Confirm',
    search: 'Search',
    plan: 'Plan',
    schedule: 'Schedule',
    move: 'Move',
    more: 'More',
    addProject: '+ Project',
    addTag: 'Tag',
    untitled: '(untitled)',
    loading: 'Loading…',
    back: 'Back',
  },
  aria: {
    sidebar: 'Sidebar',
    mainNavigation: 'Main navigation',
    content: 'Content',
    tasks: 'Tasks',
    upcomingTasks: 'Upcoming tasks',
    projectTasks: 'Project tasks',
    taskEditor: 'Task editor',
    sectionTitle: 'Section title',
    projectTitle: 'Project title',
    projectActions: 'Project actions',
    areaActions: 'Area actions',
    sectionActions: 'Section actions',
    markProjectDone: 'Mark project done',
    projectProgressOpen: 'Complete project (confirmation required). Progress: {{percent}}% ({{doneCount}}/{{totalCount}}).',
    projectProgressDone: 'Reopen project',
    projectProgressCancelled: 'Reopen cancelled project',
    taskDone: 'Done',
    collapseArea: 'Collapse area {{title}}',
    expandArea: 'Expand area {{title}}',
    removeTag: 'Remove tag {{title}}',
    manageMoreProjectTags: 'Manage {{count}} more tags',
    manageMoreAreaTags: 'Manage {{count}} more tags',
  },
  nav: {
    inbox: 'Inbox',
    today: 'Today',
    upcoming: 'Upcoming',
    anytime: 'Anytime',
    someday: 'Someday',
    logbook: 'Logbook',
    trash: 'Trash',
    settings: 'Settings',
    projects: 'Projects',
  },
  shell: {
    empty: '(empty)',
    project: 'Project',
    area: 'Area',
    areas: 'Areas',
    projectTitlePlaceholder: 'New project',
    areaTitlePlaceholder: 'New area',
    create: 'Create',
    cancel: 'Cancel',
    new: '+ New',
    task: '+ Task',
    section: '+ Section',
  },
  search: {
    title: 'Search',
    placeholder: 'Search title + notes…',
    includeLogbook: 'Include Logbook',
    resultsAriaLabel: 'Search results',
    noResults: 'No results',
    scope: 'Search scope',
    'scope.anywhere': 'Anywhere',
    'scope.inbox': 'Inbox',
    'scope.today': 'Today',
    'scope.upcoming': 'Upcoming',
    'scope.anytime': 'Anytime',
    'scope.someday': 'Someday',
    'scope.logbook': 'Logbook',
    'scope.trash': 'Trash',
    continue: 'Continue Search',
    continueHint: 'Show all results',
    resultsFor: 'Results for "{{query}}"',
    resultCount: '{{count}} results',
    results: 'Results',
  },
  logbook: {
    completedProjects: 'Completed Projects',
  },
  trash: {
    listAria: 'Trash roots',
    taskLabel: 'Task root',
    purge: 'Delete Permanently',
    purgeConfirm: 'Permanently delete this Trash item?',
    emptyAction: 'Empty Trash',
    emptyConfirm: 'Permanently delete every current Trash item?',
    emptyState: 'Trash is empty',
    rootCount: '{{count}} items',
    projectOpenCount: '{{count}} open tasks',
    restoreFromTrash: 'Restore from Trash',
  },
  settings: {
    title: 'Settings',
    generalTab: 'General',
    language: 'Language',
    languageEnglish: 'English',
    languageChinese: '中文',
    theme: 'Theme',
    themeSystem: 'System',
    themeLight: 'Light',
    themeDark: 'Dark',
    themeEffective: 'Effective: {{theme}}',
    appearance: 'Appearance',
    fontSize: 'Font size',
    fontSizeEndpointSmall: 'Small',
    fontSizeEndpointLarge: 'Large',
    fontSizeDefaultMarker: 'Default',
    fontSizeVerySmall: 'Very small',
    fontSizeSmall: 'Small',
    fontSizeSlightlySmall: 'Slightly small',
    fontSizeDefault: 'Default',
    fontSizeSlightlyLarge: 'Slightly large',
    fontSizeLarge: 'Large',
    fontSizeVeryLarge: 'Very large',
    data: 'Data',
    export: 'Export…',
    import: 'Import…',
    resetAllData: 'Reset All Data',
    resetConfirm: 'Reset all local data?',
    showInFolder: 'Show In Folder',
    about: 'About',
    version: 'Version',
    userData: 'User data',
    shortcuts: 'Shortcuts',
    openDataFolder: 'Open Data Folder',
    syncTab: 'Sync',
    syncServerUrl: 'Server Base URL',
    syncToken: 'Sync Token',
    syncEnabled: 'Enable Sync',
    syncStatus: 'Status',
    syncConnect: 'Connect',
    syncDisconnect: 'Disconnect',
    syncStatusDisabled: 'Disabled',
    syncStatusConnecting: 'Connecting...',
    syncStatusConnected: 'Connected',
    syncStatusSyncing: 'Syncing...',
    syncStatusError: 'Error: {{error}}',
    syncStatusOffline: 'Offline',
    aiTab: 'AI',
    aiEnabled: 'Enable AI assistant',
    aiBaseUrl: 'API base URL',
    aiApiKey: 'API key',
    aiApiKeyHint: 'Stored locally in plain text. Use a key with limited scope.',
    aiModel: 'Model',
    aiSave: 'Save',
    aiSaved: 'Saved.',
    aiInvalidConfig: 'Invalid AI configuration. Check the base URL and required fields.',
  },
  area: {
    renamePromptTitle: 'Rename area',
    untitled: 'New area',
    deleteConfirm: 'Delete this area? Projects and tasks under it will be soft-deleted.',
  },
  project: {
    newTitlePrompt: 'New project title',
    renamePromptTitle: 'Rename project',
    untitled: 'New project',
    completeConfirm: 'Mark project done and complete {{count}} open tasks?',
    cancel: 'Cancel project',
    cancelConfirm: 'Cancel project and cancel {{count}} open tasks?',
    deleteConfirm: 'Delete this project? Tasks and sections under it will be deleted.',
  },
  tag: {
    newPromptTitle: 'New tag',
    renamePromptTitle: 'Rename tag',
    deleteConfirm: 'Delete tag?',
  },
  section: {
    untitled: 'New section',
    deleteConfirm: 'Delete this section? Tasks in it will be moved before the section is deleted.',
    convertToProject: 'Convert to Project',
    archive: 'Archive',
    archiveConfirm: 'Archive this section and complete all open tasks in it?',
    restore: 'Restore',
  },
  task: {
    untitled: 'New task',
    titlePlaceholder: 'New task',
    notesPlaceholder: 'Notes',
    cancel: 'Cancel',
    restore: 'Restore',
    convertToProject: 'Convert to Project',
    deleteConfirm: 'Delete task?',
    removeFromProject: 'Remove from Project',
    removeFromArea: 'Remove from Area',
  },
  upcoming: {
    nextWeek: 'Next Week',
    nextMonth: 'Next Month',
    showingAfter: 'Showing tasks scheduled after {{date}}',
    empty: 'No upcoming tasks',
  },
  projectPage: {
    notesPlaceholder: 'Add notes…',
    noOpenTasks: 'No open tasks',
    completed: 'Completed',
    areaLabel: 'Area',
    reopen: 'Reopen',
    markDone: 'Mark Done',
  },
  taskEditor: {
    taskTitle: 'Task',
    statusDone: 'Done',
    statusCancelled: 'Cancelled',
    statusOpen: 'Open',
    scheduledPrefix: 'Scheduled:',
    duePrefix: 'Due:',
    tagsPrefix: 'Tags:',
    clearScheduledAria: 'Clear scheduled',
    clearDueAria: 'Clear due',
    clearTagsAria: 'Clear tags',
    popoverScheduleTitle: 'Scheduled',
    popoverDueTitle: 'Due',
    markdownPlaceholder: 'Markdown supported (stored as plain text in v0.1).',
    actionFailedTitle: 'Action failed',
    titleLabel: 'Title',
    notesLabel: 'Notes',
    projectLabel: 'Project',
    openProject: 'Open Project',
    changeProject: 'Change Project',
    sectionLabel: 'Section',
    areaLabel: 'Area',
    scheduledLabel: 'Scheduled',
    dueLabel: 'Due',
    tagsLabel: 'Tags',
    checklistLabel: 'Checklist',
    markDone: 'Mark Done',
    checklistItemDoneAria: 'Checklist item done',
    checklistItemPlaceholder: 'Checklist item',
    newTagPlaceholder: 'New tag',
    noTagsMatch: 'No items match the selected tags.',
  },
  chat: {
    panelTitle: 'AI Assistant',
    newSession: 'New Session',
    newSessionDefault: 'New Chat',
    sessions: 'Sessions',
    send: 'Send',
    stop: 'Stop',
    placeholder: 'Ask anything...',
    empty: 'Start a conversation',
    welcomeTitle: 'Milesto AI Assistant',
    welcomeHint: 'Ask Milesto AI anything to start a new conversation',
    confirmTitle: 'Confirm Action',
    confirmExecute: 'Execute',
    confirmCancel: 'Cancel',
    rollbackMessage: 'Rollback',
    rollbackMessageAria: 'Rollback to message: {{content}}',
    rollbackConfirm: 'Rollback to this earlier message? This will remove this turn and every later message.',
    rollbackConflictSummary: 'Rollback completed, but {{count}} AI side effect could not be reverted because it changed later.',
    errorDisabled: 'AI assistant is disabled. Enable it in Settings.',
    errorLoading: 'Loading AI configuration...',
    errorInvalidKey: 'Invalid API key. Please check Settings.',
    errorNetwork: 'Network connection failed. Please try again.',
    errorModelNotFound: 'Model not found. Please check your model name in Settings.',
    errorRateLimit: 'Rate limit exceeded. Please wait a moment.',
    errorGeneric: 'An error occurred. Please try again.',
  },
  errors: {
    missingAreaId: 'Missing area id.',
    missingProjectId: 'Missing project id.',
  },
  dialog: {
    export: {
      title: 'Export Milesto Data',
    },
    import: {
      title: 'Import Milesto Data',
    },
  },
  fileFilter: {
    json: 'JSON',
  },
} as const

export const messagesZhCN = {
  common: {
    none: '无',
    noneOption: '（无）',
    all: '全部',
    clear: '清除',
    close: '关闭',
    retry: '重试',
    rename: '重命名',
    delete: '删除',
    confirm: '确认',
    search: '搜索',
    plan: '计划',
    schedule: '计划',
    move: '移动',
    more: '更多',
    addProject: '+ 项目',
    addTag: '标签',
    untitled: '（无标题）',
    loading: '加载中…',
    back: '返回',
  },
  aria: {
    sidebar: '侧边栏',
    mainNavigation: '主导航',
    content: '内容',
    tasks: '任务',
    upcomingTasks: '计划任务',
    projectTasks: '项目任务',
    taskEditor: '任务编辑器',
    sectionTitle: '分组标题',
    projectTitle: '项目标题',
    projectActions: '项目操作',
    areaActions: '领域操作',
    sectionActions: '分组操作',
    markProjectDone: '标记项目完成',
    projectProgressOpen: '完成项目（需要确认）。进度：{{percent}}%（{{doneCount}}/{{totalCount}}）。',
    projectProgressDone: '重新打开项目',
    projectProgressCancelled: '重新打开已取消项目',
    taskDone: '完成',
    collapseArea: '折叠领域 {{title}}',
    expandArea: '展开领域 {{title}}',
    removeTag: '移除标签 {{title}}',
    manageMoreProjectTags: '管理另外 {{count}} 个标签',
    manageMoreAreaTags: '管理另外 {{count}} 个标签',
  },
  nav: {
    inbox: '收件箱',
    today: '今天',
    upcoming: '计划',
    anytime: '随时',
    someday: '某天',
    logbook: '日志',
    trash: '废纸篓',
    settings: '设置',
    projects: '项目',
  },
  shell: {
    empty: '(空)',
    project: '项目',
    area: '领域',
    areas: '领域',
    projectTitlePlaceholder: '新建项目',
    areaTitlePlaceholder: '新建领域',
    create: '创建',
    cancel: '取消',
    new: '+ 新建',
    task: '+ 任务',
    section: '+ 分组',
  },
  search: {
    title: '搜索',
    placeholder: '搜索标题 + 备注…',
    includeLogbook: '包含日志',
    resultsAriaLabel: '搜索结果',
    noResults: '无结果',
    scope: '搜索范围',
    'scope.anywhere': '全部',
    'scope.inbox': '收件箱',
    'scope.today': '今天',
    'scope.upcoming': '计划',
    'scope.anytime': '随时',
    'scope.someday': '某天',
    'scope.logbook': '日志',
    'scope.trash': '废纸篓',
    continue: '继续搜索',
    continueHint: '显示所有结果',
    resultsFor: '"{{query}}" 的搜索结果',
    resultCount: '{{count}} 个结果',
    results: '结果',
  },
  logbook: {
    completedProjects: '已完成项目',
  },
  trash: {
    listAria: '废纸篓根条目',
    taskLabel: '任务根条目',
    purge: '永久删除',
    purgeConfirm: '确定永久删除这个废纸篓条目？',
    emptyAction: '清空废纸篓',
    emptyConfirm: '确定永久删除当前废纸篓里的所有条目？',
    emptyState: '废纸篓为空',
    rootCount: '{{count}} 个条目',
    projectOpenCount: '{{count}} 个未完成任务',
    restoreFromTrash: '从废纸篓恢复',
  },
  settings: {
    title: '设置',
    generalTab: '常规',
    language: '语言',
    languageEnglish: 'English',
    languageChinese: '中文',
    theme: '主题',
    themeSystem: '跟随系统',
    themeLight: '浅色',
    themeDark: '深色',
    themeEffective: '当前：{{theme}}',
    appearance: '外观',
    fontSize: '字体大小',
    fontSizeEndpointSmall: '小',
    fontSizeEndpointLarge: '大',
    fontSizeDefaultMarker: '默认',
    fontSizeVerySmall: '很小',
    fontSizeSmall: '小',
    fontSizeSlightlySmall: '稍小',
    fontSizeDefault: '默认',
    fontSizeSlightlyLarge: '稍大',
    fontSizeLarge: '大',
    fontSizeVeryLarge: '很大',
    data: '数据',
    export: '导出…',
    import: '导入…',
    resetAllData: '重置所有数据',
    resetConfirm: '重置所有本地数据？',
    showInFolder: '在文件夹中显示',
    about: '关于',
    version: '版本',
    userData: '用户数据',
    shortcuts: '快捷键',
    openDataFolder: '打开数据文件夹',
    syncTab: '同步',
    syncServerUrl: '服务端基础地址',
    syncToken: '同步令牌',
    syncEnabled: '启用同步',
    syncStatus: '状态',
    syncConnect: '连接',
    syncDisconnect: '断开',
    syncStatusDisabled: '已禁用',
    syncStatusConnecting: '连接中...',
    syncStatusConnected: '已连接',
    syncStatusSyncing: '同步中...',
    syncStatusError: '错误：{{error}}',
    syncStatusOffline: '离线',
    aiTab: 'AI',
    aiEnabled: '启用 AI 助手',
    aiBaseUrl: 'API 基础地址',
    aiApiKey: 'API 密钥',
    aiApiKeyHint: '密钥以明文形式保存在本地。建议使用受限权限的密钥。',
    aiModel: '模型',
    aiSave: '保存',
    aiSaved: '已保存。',
    aiInvalidConfig: 'AI 配置无效。请检查基础地址与必填项。',
  },
  area: {
    renamePromptTitle: '重命名领域',
    untitled: '新建领域',
    deleteConfirm: '删除此领域？其下的项目和任务将被软删除。',
  },
  project: {
    newTitlePrompt: '新建项目标题',
    renamePromptTitle: '重命名项目',
    untitled: '新建项目',
    completeConfirm: '标记项目完成并完成 {{count}} 个未完成任务？',
    cancel: '取消项目',
    cancelConfirm: '取消项目并取消 {{count}} 个未完成任务？',
    deleteConfirm: '删除此项目？其下的任务与分组将被删除。',
  },
  tag: {
    newPromptTitle: '新建标签',
    renamePromptTitle: '重命名标签',
    deleteConfirm: '删除标签？',
  },
  section: {
    untitled: '新建分组',
    deleteConfirm: '删除此分组？其中的任务会先被重新归位，再删除分组。',
    convertToProject: '转化为项目',
    archive: '存档',
    archiveConfirm: '存档此分组并将其下的未完成任务全部完成？',
    restore: '恢复',
  },
  task: {
    untitled: '新建任务',
    titlePlaceholder: '新建任务',
    notesPlaceholder: '备注',
    cancel: '取消',
    restore: '恢复',
    convertToProject: '转化为项目',
    deleteConfirm: '删除任务？',
    removeFromProject: '移出项目',
    removeFromArea: '移出领域',
  },
  upcoming: {
    nextWeek: '下周',
    nextMonth: '下月',
    showingAfter: '显示 {{date}} 之后计划的任务',
    empty: '暂无计划任务',
  },
  projectPage: {
    notesPlaceholder: '添加备注…',
    noOpenTasks: '没有未完成任务',
    completed: '已完成',
    areaLabel: '领域',
    reopen: '重新打开',
    markDone: '标记完成',
  },
  taskEditor: {
    taskTitle: '任务',
    statusDone: '已完成',
    statusCancelled: '已取消',
    statusOpen: '未完成',
    scheduledPrefix: '计划：',
    duePrefix: '到期：',
    tagsPrefix: '标签：',
    clearScheduledAria: '清除计划',
    clearDueAria: '清除到期',
    clearTagsAria: '清除标签',
    popoverScheduleTitle: '计划',
    popoverDueTitle: '到期',
    markdownPlaceholder: '支持 Markdown（v0.1 中按纯文本存储）。',
    actionFailedTitle: '操作失败',
    titleLabel: '标题',
    notesLabel: '备注',
    projectLabel: '项目',
    openProject: '进入项目',
    changeProject: '更改项目',
    sectionLabel: '分组',
    areaLabel: '领域',
    scheduledLabel: '计划',
    dueLabel: '到期',
    tagsLabel: '标签',
    checklistLabel: '清单',
    markDone: '标记完成',
    checklistItemDoneAria: '清单条目完成',
    checklistItemPlaceholder: '清单条目',
    newTagPlaceholder: '新建标签',
    noTagsMatch: '没有条目匹配选中的标签。',
  },
  chat: {
    panelTitle: 'AI 助手',
    newSession: '新会话',
    newSessionDefault: '新对话',
    sessions: '会话',
    send: '发送',
    stop: '停止',
    placeholder: '随便问点什么…',
    empty: '开始对话',
    welcomeTitle: 'Milesto AI 助手',
    welcomeHint: '向 Milesto AI 助手提问以开启新会话',
    confirmTitle: '确认操作',
    confirmExecute: '执行',
    confirmCancel: '取消',
    rollbackMessage: '回滚',
    rollbackMessageAria: '回滚到消息：{{content}}',
    rollbackConfirm: '回滚到这条早期消息？这会删除这轮及之后的所有消息。',
    rollbackConflictSummary: '回滚已完成，但有 {{count}} 个 AI 副作用因后续变更未被还原。',
    errorDisabled: 'AI 助手未启用。请在设置中开启。',
    errorLoading: '正在加载 AI 配置…',
    errorInvalidKey: 'API 密钥无效。请检查设置。',
    errorNetwork: '网络连接失败。请重试。',
    errorModelNotFound: '模型不存在。请检查设置中的模型名称。',
    errorRateLimit: '请求过于频繁。请稍后再试。',
    errorGeneric: '发生错误。请重试。',
  },
  errors: {
    missingAreaId: '缺少领域 ID。',
    missingProjectId: '缺少项目 ID。',
  },
  dialog: {
    export: {
      title: '导出 Milesto 数据',
    },
    import: {
      title: '导入 Milesto 数据',
    },
  },
  fileFilter: {
    json: 'JSON',
  },
} as const

export const messageCatalogs: Record<Locale, MessageCatalog> = {
  en: messagesEn,
  'zh-CN': messagesZhCN,
}

type KeyMismatch = {
  missingInEn: string[]
  missingInZhCN: string[]
}

function collectLeafKeys(node: unknown, prefix: string, out: Set<string>) {
  if (typeof node === 'string') {
    out.add(prefix)
    return
  }

  if (node && typeof node === 'object' && !Array.isArray(node)) {
    for (const [k, v] of Object.entries(node as Record<string, unknown>)) {
      const next = prefix ? `${prefix}.${k}` : k
      collectLeafKeys(v, next, out)
    }
    return
  }

  throw new Error(`Invalid message catalog value at "${prefix}": expected string or object`)
}

export function diffCatalogKeys(a: MessageCatalog, b: MessageCatalog): string[] {
  const aKeys = new Set<string>()
  const bKeys = new Set<string>()
  collectLeafKeys(a, '', aKeys)
  collectLeafKeys(b, '', bKeys)
  return [...aKeys].filter((k) => !bKeys.has(k)).sort()
}

export function checkMessageCatalogParity(): KeyMismatch {
  const missingInZhCN = diffCatalogKeys(messagesEn, messagesZhCN)
  const missingInEn = diffCatalogKeys(messagesZhCN, messagesEn)
  return { missingInEn, missingInZhCN }
}
